...
const CrearInspector = () => {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        try {
            ...
            const result = await axios.post(`${API_URL}/inspector`, usuario);
    ...
    return (
        <div className="flex flex-col justify-center items-center text-black gap-10">
            ...
            <form className="flex flex-col w-96 gap-5 border border-white rounded-2xl p-5 bg-white text-black " onSubmit={handleSubmit(onSubmit)}>
                ...
                <div>
                    <button type="submit" className="w-full bg-blue-500 text-white p-2 rounded-md font-semibold cursor-pointer">Registrar Inspector</button>
                </div>
            </form>
        </div>
    )
